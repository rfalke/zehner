...
describe('download', function () {
    ...
    describe('file support', function () {
        ...
        it('402 will be retried and 3 times 402 results in error', function (done) {
            ...
            var promise = download.downloadOneUrlWithRetry(tmpDir, 'http://www.example.com/foo', 3, '3/14', loggers.nullLogger);
            promise.then(
                function () {
                    assert.fail();
                },
        ...
        it('unknown dns will cause an error', function (done) {
            var nocked = nock('http://www.example.com').get('/foo').
                reply(302, '', {Location: 'http://navigationshilfe1.t-online.de/dnserror?url=http://www.example.com/foo'});
            var promise = download.downloadOneUrlWithRetry(tmpDir, 'http://www.example.com/foo', 3, '3/14', loggers.nullLogger);
            promise.then(
                function () {
                    assert.fail();
